...
function normalizeSdrAppointment(row: DbRow, adminUsersById: Map<string, DbRow>): WhatsAppSdrAppointmentSummary {
  ...
  return {
    ...
    leadConfirmationStatus: asString(row.lead_confirmation_status, "pending") as WhatsAppSdrAppointmentSummary["leadConfirmationStatus"],
    leadConfirmationRequestedAt: asString(row.lead_confirmation_requested_at) || null,
    leadConfirmedAt: asString(row.lead_confirmed_at) || null,
    leadRescheduleRequestedAt: asString(row.lead_reschedule_requested_at) || null,
    confirmationDueAt: asString(row.confirmation_due_at) || null,
    confirmationSentAt: asString(row.confirmation_sent_at) || null,
    adminConfirmationNotifiedAt: asString(row.admin_confirmation_notified_at) || null,
    leadReminderSentAt: asString(row.lead_reminder_sent_at) || null,
    adminReminderSentAt: asString(row.admin_reminder_sent_at) || null,
    reminderDueAt: asString(row.reminder_due_at) || null,
    rescheduleNote: asString(row.reschedule_note) || null,
